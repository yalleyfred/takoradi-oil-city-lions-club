...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav>
        <div className="flex items-center justify-between px-52 py-3.5">
          <Link
            href={"/"}
          ...
          {/* Display Navigation on Medium screens and upwards */}
            ...
            {NavLinks.map((link) => (
              ...
              >
                <span
                  className={`${
                    path === link.path
                      ? "text-slate-900 delay-300 font-semibold transition-all duration-700 underline"
                      : ""
                  }`}
                >
                  {link.label}
          ...
          {/* Display Mobile Menu  */}
          <div
            className={`absolute border md:hidden ${visible} left-0 top-[3rem] z-50 mt-3 flex h-[calc(100vh-3.6rem)] w-full flex-col justify-between  bg-white pb-3 transition-transform duration-700 ease-in-out lg:relative lg:top-0 lg:w-full lg:translate-x-0`}
          >
